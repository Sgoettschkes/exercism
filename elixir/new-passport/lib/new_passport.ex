defmodule NewPassport do
  def get_new_passport(now, birthday, form) do
    with {:ok, enter_building_timestamp} <- enter_building(now),
         {:ok, manual} <- find_counter_information(now),
         counter_number <- manual.(birthday),
         {:ok, checksum} <- stamp_form(enter_building_timestamp, counter_number, form) do
      {:ok, get_new_passport_number(enter_building_timestamp, counter_number, checksum)}
    else
